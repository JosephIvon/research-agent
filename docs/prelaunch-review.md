...
| 多竞品登录 | 前端只有一组全局登录凭据，无法对多个竞品站点分别填写账号密码 | 首页改为多站点工作台，每个站点可单独配置登录页、用户名和密码；后端新增 `target_sites` 请求结构并保留 `urls` 兼容 |
| 报告可信度 | 低质量报告存在长小数评分、旧等级、警告行误作标题等首屏问题 | 报告页按分数重新推导等级，低分显示 F 和补数据提示，标题优先取真实 Markdown 一级标题或原查询 |
| 非技术用户体验 | 提交后只看到按钮等待，PRD 藏在二级页面，用户无法判断系统是否真的在工作 | 首页创建任务后进入 AI 调研驾驶舱，展示阶段记录；完成后进入结果中心，同页查看报告、PRD 和来源状态 |
